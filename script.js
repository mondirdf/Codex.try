...
if (contactForm && formFeedback) {
  contactForm.addEventListener('submit', (event) => {
    ...
    if (name.length < 2) {
      setError(fields.name, 'حقل الاسم: اكتب اسمك الحقيقي بحيث لا يقل عن حرفين.');
      return;
    }

    if (!email.includes('@') || !email.includes('.')) {
      setError(fields.email, 'حقل البريد الإلكتروني: أدخل بريدًا صالحًا مثل name@example.com.');
      return;
    }

    if (!service) {
      setError(fields.service, 'حقل نوع الخدمة: اختر الخدمة الأقرب لاحتياج مشروعك.');
      return;
    }

    if (message.length < 15) {
      setError(fields.message, 'حقل نبذة المشروع: أضف تفاصيل أكثر (الهدف، الجمهور، والمدة) بحد أدنى 15 حرفًا.');
      return;
    }
